...
void CWeaponMagazinedWGrenade::state_Fire(float dt) 
{
	...
	//—Ä–µ–∂–∏–º —Å—Ç—Ä–µ–ª—å–±—ã –ø–æ–¥—Å—Ç–≤–æ–ª—å–Ω–∏–∫–∞
	if(m_bGrenadeMode)
	{
		/*
		fTime					-=dt;
	...
	//—Ä–µ–∂–∏–º —Å—Ç—Ä–µ–ª—å–±—ã –æ—á–µ—Ä–µ–¥—è–º–∏
	else 
...
void CWeaponMagazinedWGrenade::ReloadMagazine() 
{
	...
	//–ø–µ—Ä–µ–∑–∞—Ä—è–¥–∫–∞ –ø–æ–¥—Å—Ç–≤–æ–ª—å–Ω–æ–≥–æ –≥—Ä–∞–Ω–∞—Ç–æ–º–µ—Ç–∞
	if(m_bGrenadeMode)
	{
...
bool CWeaponMagazinedWGrenade::Attach(PIItem pIItem, bool b_send_event)
{
	   ...
	   !xr_strcmp(*m_sGrenadeLauncherName, pIItem->object().cNameSect()))
	{
		...
		CRocketLauncher::m_fLaunchSpeed = pGrenadeLauncher->GetGrenadeVel();
 		...
 		//—É–Ω–∏—á—Ç–æ–∂–∏—Ç—å –ø–æ–¥—Å—Ç–≤–æ–ª—å–Ω–∏–∫ –∏–∑ –∏–Ω–≤–µ–Ω—Ç–∞—Ä—è
		if(b_send_event)
		{
			if (OnServer()) 
...
//‚ËÚÛ‡Î¸Ì˚Â ÙÛÌÍˆËË ‰Îˇ ÔÓË„˚‚‡ÌËˇ ‡ÌËÏ‡ˆËË HUD
	...
	if (isGuns)
	{
		if (IsGrenadeLauncherAttached())
		{
			...
			else
			{
				...
				if (IsScopeAttached() && isHUDAnimationExist("anm_shoot_aim_scope_w_gl"))
					anm_name += "_scope";
...
std::string CWeaponMagazinedWGrenade::NeedAddSuffix(std::string M)
{
	...
	if (IsGrenadeLauncherAttached())
	{
		if (m_bGrenadeMode)
		{
			...
			if (temp_name == new_name && !IsMisfire() && iAmmoElapsed2 == 0 && IsChangeAmmoType() && iAmmoElapsed != 0) //ÍÓÒÚ˚Î¸ ‰Îˇ ÍÓÌÙË„‡ SIG 550, ÔÓÚÓÏ Û·Â∏Ï
