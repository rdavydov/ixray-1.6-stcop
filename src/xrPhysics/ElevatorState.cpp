...
{
	VERIFY(m_character);
	m_start_time=inl_ph_world().GetDevice().dwTimeGlobal;
...
bool CElevatorState::StateSwitchInertion(Estate new_state)
{
	...
	Fvector p;m_character->GetFootCenter(p);
	p.sub(m_start_position);
	if(m_etable_[m_state][new_state].dist<p.magnitude()||m_etable_[m_state][new_state].time<inl_ph_world().GetDevice().dwTimeGlobal-m_start_time) return true;
	else return false;
}
